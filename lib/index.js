...
function buildApi(ctx, ptyManager, agentPtyRegistry, resolved, terminalShell, getSettings, browserBridge, automation, syncBridgeTools) {
  ...
  return {
    ...
    "browser.bookmarks.read": async () => {
      try {
        const text = await readFile3(join7(homedir3(), ".dsh", "browser-bookmarks.json"), "utf8");
    ...
    "browser.bookmarks.write": async (payload) => {
      ...
      await mkdir2(join7(homedir3(), ".dsh"), { recursive: true });
      await writeFile2(join7(homedir3(), ".dsh", "browser-bookmarks.json"), JSON.stringify(list, null, 2), "utf8");
      return { ok: true };
    },
