...
export interface SidebarConfig {
   ...
   */
  shellArgs?: string[]
  /** Fixed Chrome bridge bearer token. Empty generates/reuses ~/.dsh/ext-bridge-token. */
  browserBridgeToken?: string
  /** Browser action timeout in milliseconds. */
  browserBridgeToolTimeoutMs?: number
  /** Maximum characters returned by one extension snapshot. */
  browserBridgeSnapshotMaxChars?: number
  /** Maximum numbered interactive items in one snapshot. */
  browserBridgeMaxInteractiveItems?: number
}
...
export const Config: z<SidebarConfig> = z.object({
  ...
  browserBridgeToolTimeoutMs: z.number().step(1).min(1).default(90_000),
  browserBridgeSnapshotMaxChars: z.number().step(1).min(500).default(32_000),
...
export function resolveSidebarConfig(config: SidebarConfig | undefined): ResolvedSidebarConfig {
  return {
    ...
    browserBridgeToolTimeoutMs: config?.browserBridgeToolTimeoutMs ?? 90_000,
    browserBridgeSnapshotMaxChars: config?.browserBridgeSnapshotMaxChars ?? 32_000,
