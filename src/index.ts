...
import { BRIDGE_CONFIG_PATH, BRIDGE_PATH } from './browser-bridge/protocol.ts'
import { resolveToken } from './browser-bridge/token.ts'
import { registerBrowserTools } from './browser-bridge/tools.ts'
import { BrowserAutomationService } from './browser-automation.ts'
import { registerWebviewTools } from './browser-automation-tools.ts'
...
function buildApi(
  ...
  browserBridge: BrowserBridgeServer,
  automation: BrowserAutomationService,
  syncBridgeTools: () => void,
  ...
  return {
    ...
    'browser.registerWebContents': (payload) => {
      const sessionId = requireString(payload, 'sessionId')
      const webContentsId = (payload as { webContentsId?: unknown }).webContentsId
    ...
    'browser.bridge.navigate': async (payload) => {
      ...
      try {
        const result = await browserBridge.requestTool('browser_navigate', { url: parsed.href }, AbortSignal.timeout(15_000))
 ...
 */
export async function apply(ctx: Context, config?: SidebarConfig): Promise<void> {
  ...
  const tokenResult = await resolveToken(resolved.browserBridgeToken || undefined)
  ...
  // Embedded <webview> automation (DSH Desktop): host half runs inside the
  // Electron main process (the desktop bootstraps the Host Cordis root), so
  // webContents.fromId() reaches the sidebar browser's guest directly.
  ...
  ctx.effect(() => ctx.webServer.registerUpgrade({
    ...
    handler: (req, socket, head) => {
      browserBridge.handleUpgrade(req as unknown as IncomingMessage, socket as unknown as Duplex, head as Buffer)
    },
  ...
  // Chrome 扩展桥工具：只在扩展连接后才注册，避免模型误用并报
  // "no browser extension is connected"。client 每 5s 轮询
  // browser.bridge.status（BrowserView 的 bridge 心跳），host 在这个
  // handler 里同步注册/注销 —— 无扩展时模型只见 webview_*（内嵌浏览器）。
  let disposeBridgeTools: (() => void) | null = null
  const syncBridgeTools = (): void => {
  ...
  // The bridge must stay usable when the published Context type has no logger.
