...
export const api = {
  ...
  browserBridgeStatus: (signal?: AbortSignal) =>
    call<{ connected: boolean }>('browser.bridge.status', {}, signal),
  /** Navigate the extension-controlled real Chrome tab. */
  browserBridgeNavigate: (url: string, signal?: AbortSignal) =>
    call<{ connected: true; result: unknown }>('browser.bridge.navigate', { url }, signal),
