...
import {
  ...
  resizeSplitIn, setBottomHeight, setWidth, swapPanelSide, toggleBottomPanel, toggleExpanded, togglePanel,
  type DropZone, type SidebarState, type SidebarStore, type SidebarTab, type SplitNode,
} from './state.ts'
import { IconPanelBottomOutline16, IconPanelRightOutline16, IconPanelSwapOutline16 } from './icons.tsx'
...
export function Sidebar(props: { ctx: Context; store: SidebarStore }) {
  ...
  const [centerRect, setCenterRect] = useState({ left: 0, right: 0, shellLeft: 0 })
  ...
  const measureCenter = useCallback((): void => {
    ...
    // shellLeft is the stable edge after the app's own workspace navigation.
    // It anchors a left-positioned panel while the conversation column yields
    // width to it. The bottom panel follows the measured conversation edges.
    setCenterRect(prev =>
      prev.left === rect.left && prev.right === rect.right && prev.shellLeft === shellLeft
        ? prev
        : { left: rect.left, right: rect.right, shellLeft })
  }, [])
  useEffect(() => {
    let disposed = false
  ...
  if (state === undefined || sessionId === undefined) {
    return (
      <div className={css.toggleCluster} data-side={panelSide}>
        {!narrow && (
  ...
  return (
    <>
      ...
      */}
      <div className={css.toggleCluster} data-side={panelSide}>
        {!narrow && (
          <Tooltip label={t('swapPanelSide')} side="bottom" delayMs={500}>
            <button
              type="button"
              className={css.toggleButton}
              aria-label={t('swapPanelSide')}
              onClick={() => { store.reduce(swapPanelSide) }}
            >
              <IconPanelSwapOutline16 />
            </button>
          </Tooltip>
        )}
        {/*
      ...
      */}
      <div
        ...
        style={{
          ...
          left: !narrow && panelSide === 'left' ? centerRect.shellLeft : undefined,
          right: narrow || panelSide === 'right' ? 0 : undefined,
        }}
        data-side={panelSide}
      ...
      */}
      ...
      <div
        ...
        style={{
          ...
          right: panelSide === 'left'
            ? window.innerWidth - centerRect.right
            : window.innerWidth - centerRect.right,
          // The seam follows whichever edge owns the vertical panel.
          borderLeft: state.panelOpen && panelSide === 'left' ? '1px solid var(--dsw-alias-border-l2)' : undefined,
          borderRight: state.panelOpen && panelSide === 'right' ? '1px solid var(--dsw-alias-border-l2)' : undefined,
        }}
